...
#define ExtractField(root, field) \
    if (root[#field].isNull()) { LogInfo("{} is null.", #field); \
        encodeDealfileJson(ShareFail, resp_json); \
        return -1; } \
...
//删除文件
int handleDeleteFile(const string &user, const string &md5, const string &filename) {
    /*
    a)先判断此文件是否已经分享
...
int ApiDealfile(string &url, string &post_data, string &resp_json)
{
    ...
    if (!res)
    {
        ...
        encodeDealfileJson(HTTP_RESP_FAIL, resp_json);
        return -1;
    }
    //解析json
    ...
    if(strcmp(cmd, "del") == 0) {
        ...
        ret = handleDeleteFile(user, md5, filename);
        if (ret == HTTP_RESP_OK)
            encodeDealfileJson(ShareOk, resp_json);
        else
            encodeDealfileJson(ShareFail, resp_json);

    } else if (strcmp(cmd, "pv") == 0) {
        ret = handlePvFile(user, md5, filename);
        encodeDealfileJson(ret, resp_json);
    } if (strcmp(cmd, "share") == 0) {
        ret = handleShareFile(user, md5, filename);
        encodeDealfileJson(ret, resp_json);
    } else {
